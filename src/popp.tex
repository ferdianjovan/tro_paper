...
The rate parameter $\lambda$ of the POPP model can be inferred by marginalising over all possible true count values $c_i$ in $P(\lambda ; c_i)$ and in the  distribution of true counts given sensed counts $P(c_i ; \mathbf{s_i})$. 
% 
...
\begin{equation}
	...
	\begin{tabular}{r@{=}l}
		$P(\lambda ; \mathbf{s})$ &  $\displaystyle\sum_{c_1=0}^{\infty} \ldots \displaystyle\sum_{c_n=0}^{\infty} P(\lambda ; \mathbf{c}) ~ P(\mathbf{c} ; \mathbf{s})$ \\
	\end{tabular}
\end{equation}
\noindent where true count probabilities, $P(\lambda ; \mathbf{c})$, can be drawn from the original FOPP definition:
\begin{equation*}
	\begin{tabular}{r@{ = }l}
		$P(\lambda ; \mathbf{c})$ & $Gam\Bigg(\lambda ; \displaystyle\sum_{i=1}^{n} c_i + \alpha, n + \beta \Bigg)$
	\end{tabular}
\end{equation*}
...
\begin{equation}
	...
	\begin{tabular}{r@{=}l}
	$P(\mathbf{s_i} ; c_i)$ & $\displaystyle\prod_{j=1}^{m} P(s_{j,i} ; c_i)$ \\ 
	\end{tabular}
\end{equation}

Using this, the probability of a particular sequence of $n$ counts, given a sequence of $n$ observations each from $m$ sensors, $P(\mathbf{c} ; \mathbf{s})$, can be defined as:

\begin{equation}
    \label{eq:occurrences_likelihood}
    \begin{tabular}{r@{ $\varpropto$ }l}
        $P(\mathbf{c} ; \mathbf{s})$ & $P(\mathbf{s_1}, \ldots, \mathbf{s_n} ; \mathbf{c}) ~ P(\mathbf{c})$ \\ [1ex]
        & $\displaystyle\prod_{i=1}^{n} P(\mathbf{s_i} ; c_i) ~ P(c_i)$ \\ [2ex]
        & $\displaystyle\prod_{i=1}^{n} \displaystyle\prod_{j=1}^{m} P(s_{j,i} ; c_i) ~ P(c_i ; \mathbf{c_{-1}})$
    \end{tabular}
\end{equation}

\noindent where $\mathbf{c_{-1}} = c_{i-1}, \ldots, c_1$, and $P(c_i ; \mathbf{c_{-1}})$ can be calculated using a negative binomial distribution:

\begin{equation}
	\label{eq:unconditional_xi}
	\begin{tabular}{r@{=}l}
		$P(c_i ; \mathbf{c_{-1}})$ & $\displaystyle\int_{\lambda=0}^{\infty} P(c_i ; \lambda) ~ P(\lambda ; \mathbf{c_{-1}}) ~d\lambda$ \\ [2ex]
...
To address this difficulty, in~\cite{jovan18a} we proposed three estimators, each of which offers an approximation to the true posterior $P(\lambda ; \mathbf{s})$. The estimators are: \\
(1) a \textit{gamma filter}, which approximates Eqn.~\ref{eq:marginal_occurrences} with a single gamma minimising the KL-divergence $D_{KL}(P(\lambda ; \mathbf{s}) \mid \mid Gam(\lambda \mid \alpha, \beta))$ by gradient descent. The accuracy of this filter deteriorates as sensor reliability degrades; \\
(2) a \textit{histogram filter}, which approximates Eqn.~\ref{eq:marginal_occurrences} with a discrete distribution $Q(\lambda ; \mathbf{s})$ by quantising $\lambda$ into discrete values. The advantage of this filter over the gamma filter is that it can track the posterior to an arbitrary fidelity via a finer quantisation. Its disadvantage is an increase in computation time by at least ten fold compared to the gamma filter; \\
(3) a \textit{switching filter}, which approximates Eqn.~\ref{eq:marginal_occurrences} either by a gamma filter or by a histogram filter depending on whether $P(\lambda ; \mathbf{s})$  resembles a gamma distribution and can be approximated by the gamma filter via KL-divergence $D_{KL}(P(\lambda ; \mathbf{s}) \mid \mid Gam(\lambda \mid \alpha, \beta))$.
...
In general (and in our experimental work from Section~\ref{sec:evasim} onwards) we recommend the use of the switching filter as the estimator to the true posterior $P(\lambda ; \mathbf{s})$ because it combines the best of both the gamma filter (fast calculation) and the histogram filter (accurate approximation) with minimum loss in similarity to the true posterior $P(\lambda ; \mathbf{s})$. Figure \ref{fig:kl_div_tpr_tnr_var} shows KL-divergence between the gamma and switching filters to the true posterior over different sensor reliabilities using simulated data. Note that the histogram filter was not included because it perfectly tracked $P(\lambda ; \mathbf{s})$, i.e., $D_{KL}(P(\lambda ; \mathbf{s}) \mid \mid Q(\lambda ; \mathbf{s})) \approx 0$. A more detailed presentation of these estimators is given in~\cite{jovan18a}.