...
\end{equation}
\noindent Both $P^+$ and $P^-$\footnote{$\mathbf d$ is dropped in the representation unless the context is not clear.} have $2^m$ elements. These two probabilities represent \textit{true positive rates} and \textit{true negative rates}  as $\tau$ and $\xi$ for the POPP model. Similar to $\tau$ and $\xi$, $P^+$ and $P^-$ are estimated from both detections of each sensor and the corresponding actual (non-)event as ground truth. However, unlike $\tau$ and $\xi$ which are sensor specific, $P^+$ and $P^-$ consider all combinations of binary detections coming from each sensor given the true event. This means that $P^+$ and $P^-$ are typically larger in the number of elements than $\tau$ and $\xi$ and the gaps grow as the number of sensors increase. For example, with three sensors there are $\tau_1, \tau_2, \tau_3$ for true positive rates and $\xi_1, \xi_2, \xi_3$ for true negative rates in the POPP model. In comparison, there will be eight elements for $P^+$ and eight elements for $P^-$ in the C-POPP model. While $\tau_1, \tau_2, \tau_3, \xi_1, \xi_2, \xi_3$ can be roughly estimated from less than a hundred detections and their corresponding events, $P^+$ and $P^-$ may need more than a few hundreds of detections together with their corresponding events.
