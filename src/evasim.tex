...
Evaluation on synthetic data is used to establish the benefit of POPP and its variations over the The uncorrected estimate according to the FOPP model in estimating the arrival rate $\lambda$ of a Poisson process. With synthetic data, sensor reliability can be controlled, and the true $\lambda$ and the true counts $c_i$ are known in advance for each sample.
